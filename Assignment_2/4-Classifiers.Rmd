...
We will use the UniversalBank datasets.
...
Assume that we are interested in creating a training and validation set from this dataset. 
...
```{r}
library(dplyr)
gsub(" ",".",colnames("Personal Loan"),("Securities Account"),("CD Account"))
m_universalbank <- select(UniversalBank,Age,Experience,Income,Family,CCAvg,Education,Mortgage,Online,CreditCard) # Select a subset of variables

set.seed(15)
Test_Index = createDataPartition(m_universalbank$Age,p=0.6, list=FALSE) # 60% reserved for Test
Test_Data = m_universalbank[Test_Index,]
TraVal_Data = m_universalbank[-Test_Index,] # Validation and Training data is rest

Train_Index = createDataPartition(TraVal_Data$Age,p=0.4, list=FALSE) # 40% of remaining data as training
...
# use preProcess() 
norm.values <- preProcess(Train_Data[, 2:3], method=c("center", "scale"))

train.norm.df[, 2:3] <- predict(norm.values, Train_Data[, 2:3]) # Replace first two columns with normalized values
valid.norm.df[, 2:3] <- predict(norm.values, Validation_Data[, 2:3])
traval.norm.df[, 2:3] <- predict(norm.values, traval.norm.df[, 2:3])
test.norm.df <- predict(norm.values, Test_Data)

summary(train.norm.df)
var(train.norm.df[, 2:3])
summary(valid.norm.df)
var(valid.norm.df[, 2:3])
...
library(FNN)
library(class)
nn <- knn(train = train.norm.df[, 2:3], test = test.norm.df, 
          cl = train.norm.df[, 3], k = 1, prob=TRUE) # We use k = 1, and Urban is the Y
...
for(i in 1:14) {
  knn.pred <- knn(train.norm.df[, 2:3], valid.norm.df[, 2:3], 
                  cl = train.norm.df[, 3], k = i, prob=TRUE)
...
norm.values <- preProcess(TraVal_Data[, 2:3], method=c("center", "scale")) # Use combined set to normalize

traval.norm.df[, 2:3] <- predict(norm.values, TraVal_Data[, 2:3])
test.norm.df[, 2:3] <- predict(norm.values, Test_Data[, 2:3])
...
```{r}
knn.pred.new <- knn(traval.norm.df[, 2:3], test.norm.df, 
                    cl = traval.norm.df[, 2:3], k = 1)
...
norm_model<-preProcess(UniversalBank, method = c("center", "scale")) # We are using range here, but we can easily switch to c("center", "scale")
UniversalBank_normalized<-predict(norm_model,UniversalBank)
summary(UniversalBank_normalized)
sd(UniversalBank_normalized$ID)
```
...
set.seed(123)
model<-train(UniversalBank~ID+Age, data=UniversalBank_normalized, method="knn")
model
```
...
model<-train(UniversalBank~ID+Age, data=UniversalBank_normalized, 
...
set.seed(123)
Serach_grid <- expand.grid(k=c(2,7,9,15))
model<-train(UniversalBank~ID+Income, data=UniversalBank, 
             method="knn", tuneGrid=Serach_grid,
             preProcess='range')
model
```
...
Index_Train<-createDataPartition(UniversalBank_normalized$Age, p=0.8, list=FALSE)
...
Test_Predictors<-Test[,1]

Train_labels <-UniversalBank[,1] 
Test_labels  <-UniversalBank_normalized[,1] 
```
